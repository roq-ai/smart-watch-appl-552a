...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'team-member', 'administrator'];
  ...
  const { data, error, isLoading } = useSWR<ManufacturerInterface[]>(
    ...
    () => getManufacturers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Watches', path: '/watches', entity: 'watch', service: AccessServiceEnum.PROJECT, icon: FiWatch },
