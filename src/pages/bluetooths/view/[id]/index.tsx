...
import { getBluetoothById } from 'apiSdk/bluetooths';
import { BluetoothInterface } from 'interfaces/bluetooth';

function BluetoothViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('bluetooth', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/bluetooths/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Connectivity" text={data?.connectivity} />
                ...
                <FormListItem label="Bandwidth" text={data?.bandwidth} />

                {hasAccess('watch', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Watch"
                    text={
                      <Link as={NextLink} href={`/watches/view/${data?.watch?.id}`}>
                        {data?.watch?.model}
                      </Link>
                    }
                  />
