...
import { getBluetooths } from 'apiSdk/bluetooths';
import { BluetoothInterface } from 'interfaces/bluetooth';

import { BluetoothListPage as ShowCaseList } from 'pages/bluetooths';
...
function DashboardPage() {
  ...
  const watchStat = useSWR<PaginatedInterface<WatchInterface>>(
    () => `/watches`,
    () => getWatches({ limit: 1, offset: 0 }),
  );

  const bluetoothStat = useSWR<PaginatedInterface<BluetoothInterface>>(
    () => `/bluetooths`,
    () => getBluetooths({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Watches',
      result: watchStat?.data?.totalCount,
      isLoading: watchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bluetooths',
      result: bluetoothStat?.data?.totalCount,
      isLoading: bluetoothStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bluetooths`} passHref legacyBehavior>
                  <Button
